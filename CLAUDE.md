# BWA-Mood — Read this before doing anything
...
can clone, browse, and reason about BWA-Mood on its own without
seeing the rest of the BWA monorepo.

All real work on BWA-Mood happens inside the BWA monorepo at:

```
~/Developer/bwa-fx-ecosystem/plugins/mood/
    dsp/           ← authoritative DSP implementation (THE source of truth)
    ui/            ← UI surface (Metal-rendered)
    plugin/        ← VST3 wrapper → builds BWA-Mood.vst3
    manifest.json  ← name, controls, license tier, ecosystem benefits
```

If you've been asked to "work on BWA-Mood," **navigate to the
...
1. **Standalone VST3** (this repo's release artifact) — `BWA-Mood.vst3`
...
- **Category:** B (Generative / Emotional — UI plugin)
- **Tech tree tier:** Tier 5
- **Role:** Project-level emotion state writer. Reads/writes shared/emotion/.
- **Pain attacked:** Make emotion a first-class project-level state. No DAW does this; without it, every micro-decision is in an emotional vacuum.
- **Scope verdict:** project-global (it's the thing every other plugin reads)
- **Band tier behavior:** Not tier-gated. The emotion bus is foundational.
...
4. **[`BWA-Architecture/docs/MARKET-PAIN-AND-ATTACKS.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/MARKET-PAIN-AND-ATTACKS.md)**
   — the row for BWA-Mood. Internalize the pain + the attack before
...
- ❌ Committing to this repo. Go to the monorepo (`plugins/mood/`).
...
- ❌ Adding new shared/* modules without checking if the capability folds
  ...
  Methodology Rule 3."
- ❌ Skipping the BWA-Mood row in MARKET-PAIN-AND-ATTACKS.md. If the
  proposed work doesn't advance the BWA Attack for this plugin, push back.

## What "done" looks like

A `BWA-Mood.vst3` that:
...
```bash
git subtree push --prefix=plugins/mood \
  git@github.com:vmcguire/BWA-Mood.git main
```

Then on GitHub:

1. Create a Release from `main` in this repo.
2. Attach the notarized `BWA-Mood.vst3` zip.
3. Reference the monorepo commit SHA in the release notes.

---

_End of LLM onboarding for BWA-Mood._
